...
2. **feature/* → CI Pipeline** — Push lên nhánh feature/* kích hoạt (push trigger) workflow CI (ci.yml) để kiểm tra chất lượng code.
3. **main → CD Pipeline** — Khi code được merge vào nhánh main, workflow CD (cd.yml) được kích hoạt (merge trigger) để triển khai.
4. **CD → DockerHub** — CD build Docker image của ứng dụng và push image (goldenowl-app:latest) lên DockerHub registry.
5. **CD → AWS EC2** — CD dùng SSH kết nối vào EC2 để thực hiện lệnh deploy (pull image mới và chạy lại container).
